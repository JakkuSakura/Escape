...
namespace Escape {
    ...
    Escape::BulletSystem::BulletSystem() {
    }

    void Escape::BulletSystem::initialize() {
    ...
    void Escape::BulletSystem::fire(Entity firer, BulletType type, float angle, float speed, float damage) {
        ...
        world->assign<BulletData>(bullet, BulletData{firer_id : world->get<ID>(
                firer).id, type : type, damage : damage, hit : false});
    ...
    Escape::WeaponSystem::WeaponSystem() {
    ...
    void Escape::WeaponSystem::initialize() {
        ECSSystem::initialize();
        bullet_system = findSystem<BulletSystem>();
        timeserver = findSystem<TimeServer>();
    }

    void Escape::WeaponSystem::fire(Entity ent, float angle) {
